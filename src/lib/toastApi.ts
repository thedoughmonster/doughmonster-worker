 ...
 */
export async function toastGet<T>(env: EnvDeps, path: string, query: Record<string, string> = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Toast GET ${url.pathname} failed: ${res.status}${body ? ` - ${body}` : ""}`);
  }
